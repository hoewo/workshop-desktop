...
- Desktop 直接创建、记录转待办和 AI/CLI 创建提议共用同一任务创建服务；Desktop 创建面使用独立窗口，任务必须明确内容和项目成员负责人，项目标签可选，初始状态为 `pending`。
...
- 窗口位置/尺寸与列表折叠、搜索、编辑等内容状态分离。排列约束持续到用户手动移动、缩放或进入受保护编辑状态；用户手动放大的记录窗口不能因正文变化被内容适配缩小，创建面、未保存草稿和弹出选择面不能在整理中被强制移动或压缩。
- 项目工作区关闭按钮的主点击只关闭当前窗口；右键菜单可以按当前项目关闭任务详情、记录详情或全部相关窗口，关闭请求先交给各详情窗口执行既有保存流程。
