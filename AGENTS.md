...
本文件是 Workshop Desktop 开发团队和 AI 协作者的内部协作手册。它可以随公开仓库分发，但内容面向开发执行，不承担产品介绍或用户上手职责。

本 repo 使用最小文档结构支持 AI 开发。普通代码任务开始时不要默认启动完整治理流程。

## 文档受众

- `README.md` 是 GitHub 公开项目首页，优先服务首次访问者、使用者和潜在贡献者，只保留产品定位、下载安装、核心能力、基本使用、数据边界和开发入口。
- `AGENTS.md` 保存开发团队与 AI 的内部执行规则，包括 repo 上下文、项目 ID、跨仓库检查、记录/任务写入边界、CLI 使用和提交前要求。
- `docs/architecture.md`、`docs/domain.md`、`docs/testing.md`、`docs/release.md` 和 `docs/decisions.md` 分别维护稳定的架构、领域、验证、发布和决策事实；不要把它们的完整内容复制回 README 或本文件。
- README 不写内部项目 ID、agent token、RPC/confirmation 实现细节、个人机器兼容说明或完整维护命令。需要公开的用户行为先确认已由当前代码支持。
- 凭证和敏感配置不进入任何 repo 文档；只记录变量名、权限边界和配置位置。
