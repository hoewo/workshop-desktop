...
- 首页、项目工作区和记录“转为待办”都打开独立创建窗口，不改变来源窗口尺寸；创建或取消后关闭创建窗口，记录转任务成功后关闭来源记录窗口并打开新任务详情。
...
- 项目工作区关闭按钮主点击只关闭当前窗口；右键可分别关闭本项目任务详情、记录详情或全部相关窗口，存在未保存内容时仍走各详情窗口既有保存流程。
...
- 手动放大记录详情窗口后继续输入正文，内容适配不能把窗口缩回正文最小高度；重新打开窗口后仍按正常初始尺寸计算。
